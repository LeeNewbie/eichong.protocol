...
public class StopCarOrganService  {
	 ...
	 * 3充电桩充电记录
	 */
	public static void realData(SCOEp ep)
	{
		...
		String epCode = ep.getEpCode();
		int gunNo = ep.getGunNo();
		int workStatus = ep.getWorkStatus();
		int gunConnStatus = ep.getGunConnStatus();
		int faultCode = ep.getFaultCode();
		long createTime = DateUtil.getCurrentSeconds();
		BigDecimal vol_a = ep.getVol_a();
		BigDecimal vol_b = ep.getVol_b();
		BigDecimal vol_c = ep.getVol_c();
		BigDecimal cur_a = ep.getCur_a();
		BigDecimal cur_b = ep.getCur_b();
		BigDecimal cur_c = ep.getCur_c();
		BigDecimal power = ep.getPower();
		BigDecimal volt = ep.getVolt();
		BigDecimal cur = ep.getCur();
		int soc = ep.getSoc();
		int restTime = ep.getRestTime();
...
//      15.剩余充电时长
	 ...
	 * 3充电桩充电记录
	 */
	public static void chargeRecord(SCOEp ep)
	{
		...
		String epCode = ep.getEpCode();
		int gunNo = ep.getGunNo();
		BigDecimal bdmeterNum = ep.getBdmeterNum();
		long lStartTime = ep.getlStartTime();
		long lEndTime = ep.getlEndTime();
		String partnerKey = GameConfig.scoCfg.getPartnerKey();
		String partnerId = GameConfig.scoCfg.getPartnerId();
