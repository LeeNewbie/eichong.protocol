...
public class AnalyzeProtocol{
	...
	public static  byte[] Package(byte[] data,byte cos,short cmdtype) {
		...
		try {
			...
			logger.error("Package exception 1,e.StackTrace:{}",e.getStackTrace());
		}
		...
		try {
			...
			logger.error("Package exception 2,e.StackTrace:{}",e.getStackTrace());
		}
    ...
    public static  byte[] do_one_bit_yx(String epCode,int epGunNo,int currentType,Map<Integer, SingleInfo> pointMap) { 		
		...
		while (iter.hasNext()) {
			...
			logger.debug("do_one_bit_yx,realData sendToMinitor dataType:1=oneBitYx,epCode:{},epGunNo:{},address:{},value:{}",
					new Object[]{epCode,epGunNo,pointAddr,info.getIntValue()});

		} 
		}catch (IOException e) {
			logger.error("do_one_bit_yx exception,e.StackTrace:{}",e.getStackTrace());
		}
    ...
    public static  byte[] do_two_bit_yx(String epCode,int epGunNo,int currentType,Map<Integer, SingleInfo> pointMap) { 		
		...
		while (iter.hasNext()) {
			...
			logger.debug("do_two_bit_yx,realData sendToMinitor dataType:2=twoBitYx,epCode:{},epGunNo:{},address:{},value:{}",
					new Object[]{epCode,epGunNo,pointAddr,info.getIntValue()});

		} 
		}catch (IOException e) {
			logger.error("do_two_bit_yx exception,e.StackTrace:{}",e.getStackTrace());
		}
    ...
    public static  byte[] do_yc(String epCode,int epGunNo,int currentType,Map<Integer, SingleInfo> pointMap) { 		
		...
		while (iter.hasNext()) {
			...
			if(pointAddr == 1)
			{
				logger.info("realData sendToMinitor dataType:3=yc,epCode:{},epGunNo:{},address:{},value:{}",
						new Object[]{epCode,epGunNo,pointAddr,info.getIntValue()});
			}
			else
			    logger.debug("realData sendToMinitor dataType:3=yc,epCode:{},epGunNo:{},address:{},value:{}",
					new Object[]{epCode,epGunNo,pointAddr,info.getIntValue()});

		} 
		}catch (IOException e) {
			logger.error("realData do_yc exception,e.StackTrace:{}",e.getStackTrace());
		}
    ...
    public static  byte[] do_var_yc(String epCode,int epGunNo,int currentType,Map<Integer, SingleInfo> pointMap) { 		
		...
		while (iter.hasNext()) {
			...
			logger.debug("do_var_yc,realData sendToMinitor dataType:4=varYc,epCode:{},epGunNo:{},address:{},value:{}",
					new Object[]{epCode,epGunNo,pointAddr,info.getIntValue()});

		} 
		}catch (IOException e) {
			logger.error("do_var_yc exception,e.StackTrace:{}",e.getStackTrace());
		}
    ...
    public static  byte[] do_heart() {
		...
		try {
			...
			logger.error("do_heart exception,e.StackTrace:{}",e.getStackTrace());
		}
