...
import com.epcentre.service.AppClientService;
...
import com.epcentre.service.GunPointMap;
...
public class ImitateConsumeService {
    	   ...
    	   if(error>0)
    		...
    		if(userRealInfo.getPassword().compareTo(pwMd5) !=0)
             ...
             UserCache u = UserService.getUserCache( account );
       		...
       		byte[] cmdTimes = WmIce104Util.timeToByte();
       		int errorCode = EpChargeService.apiStartElectric(epCode, epGunNo, u,"",null, StringUtil.repeat("0", 12), (short)chargetype,  frozenAmt,payMode,1000,1,"",cmdTimes);
       		if(errorCode>0)
       		{
       			return "errorCode:"+ errorCode;
       		}
	...
	public static String testStopCharge( Map<String, List<String>> params)
	{
    	...
    	int errorCode = EpChargeService.apiStopElectric(epCode, epGunNo,  UserConstant.ORG_I_CHARGE,u.getId(),u.getAccount(),0,apiClientIp);
    	if(errorCode>0)
		{
			return "errorCode:"+ errorCode;
		}
	...
	public static String force_update_ep_hex( Map<String, List<String>> params)
	{
				...
				)
		{
			...
			{
				logger.error("[handleVersion],accept Monitor updateHexFile,softVersion error");
				return "softVersion error";
			}
			if(bom.splitHardwareVersion()>0)
			{
				logger.error("[handleVersion],accept Monitor updateHexFile,hardwareVersion error");
			...
			logger.info("[handleVersion],accept Monitor updateHexFile,epCode:{},stationAddr:{}",code,stationAddr);
	...
	public static String testCardAuth( Map<String, List<String>> params)
    {
		...
		if(vals!=null && vals.size()>=1)
		{
			...
			String chOrCode = EpChargeService.makeChargeOrderNo(123,123);
	...
	public static String createIdentyCode( Map<String, List<String>> params)
    {
				...
				&& vals1!=null && vals1.size()>=1)
		{
			...
			EpService.handleEpIdentyCodeQuery(null,epCode, gunno,(byte)0,(byte)0,(byte)0);
	...
	public static String setStopCarOrganValid( Map<String, List<String>> params)
    {
        ...
        int flag = 0;
        int time =0;
        List<String> vals = params.get("time");
		List<String> vals1 = params.get("flag");
				...
				&& vals1!=null && vals1.size()>=1)
		{
			...
			time = Integer.parseInt(vals.get(0));
			flag = Integer.parseInt(vals1.get(0));
			if(GameConfig.scoCfg ==null)
				return "GameConfig.scoCfg is null,set fail";
			GameConfig.scoCfg.setValid(flag);
			GameConfig.scoCfg.setSendRealDataCyc(time);
			
			String str = "stopCarOrganValid:"+GameConfig.scoCfg.getValid()+"\nstopCarOrganSendCycle:"
			   +GameConfig.scoCfg.getSendRealDataCyc();
			return str;
		}
		return "params invalid";
    }
	...
	public static String getCacheSize( Map<String, List<String>> params)
    {
		...
		String epStatMsg =  EpService.getCacheSize();
		
		String gunStatMsg =  EpGunService.getCacheSize();
	
		String PhoneStatMsg =  PhoneService.getCacheSize();
		
		String apiStatMsg =  AppClientService.getCacheSize();

		sb.append(epCommClientStatMsg).append(stationStatMsg).append(epStatMsg).append(gunStatMsg).append(PhoneStatMsg).append(apiStatMsg);
	...
	public static String queryFlashRam( Map<String, List<String>> params)
    {
	   ...
	   if(vals!=null && vals.size()>=1 )
	   {
		   ...
		   byte[] cmdTimes = WmIce104Util.timeToByte();
		   byte[] data= EpEncodeProtocol.do_get_flashRam(epCode, (short)stationAddr, (short)type, startPos,(short)len);
	...
	public static String getGameConfig( Map<String, List<String>> params)
    {
		try
		{
	        ...
	        sb.append("GameConfig.resendEpMsgTime=").append(GameConfig.resendEpMsgTime).append("\n\n");
	        sb.append("GameConfig.resendEpMsgFlag=").append(GameConfig.resendEpMsgFlag).append("\n\n");
	        sb.append("GameConfig.sms=").append(GameConfig.sms).append("\n\n");
	        sb.append("GameConfig.stopCarOrganValid=").append(GameConfig.scoCfg.getValid()).append("\n\n");
	        sb.append("GameConfig.printEpMsg=").append(GameConfig.printEpMsg).append("\n\n");
	        sb.append("GameConfig.printPhoneMsg=").append(GameConfig.printPhoneMsg).append("\n\n");
			
			return sb.toString();	
		}
		catch (Exception e) {
			logger.error("getGameConfig exception,e.getMessage():{}",e.getMessage());
		}
	...
	public static String setVinCode( Map<String, List<String>> params)
    {
				...
				&&vals2!=null && vals2.size()>=1)
		{
			...
			ElectricPileCache epCache = EpService.getEpByCode(code);
			if(epCache==null)
			{
				return " setVinCode  fail epCache ==null!";
			}
			if(epCache.getCurrentType() == EpConstant.EP_AC_TYPE)
			{
				return " setVinCode ep is AC";
			}
			EpGunCache epGunCache = EpGunService.getEpGunCache(code, gunno);
			if(epGunCache ==null)
			{
			...
			singleInfo.setAddress(EpProtocolConstant.YC_VAR_CAR_VIN);
			singleInfo.setStrValue(vinCode);
			gunPointMap.getPointMap().put(EpProtocolConstant.YC_VAR_CAR_VIN, singleInfo);
