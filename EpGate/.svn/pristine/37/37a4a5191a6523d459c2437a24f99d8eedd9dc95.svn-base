...
public class EpNettyDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext channelHandlerContext,
		...
		{
			logger.debug("epChannelHandler,decode readableBytes:{} < apci header 7,channel:{}", readableBytes,channelHandlerContext.channel());
			return;
		}
		byte bFlag104 = 0x68;
		...
		if(pos < 0 )//没找到，全部读掉
		{
			...
			logger.debug("epChannelHandler,decode not find flag header 0x68,discardLen:{},channel:{}",discardLen,channelHandlerContext.channel());
		...
		if(pos>0)
		{
			...
			logger.debug("epChannelHandler,decode find flag header 0x68 at pos:{},channel:{}",pos,channelHandlerContext.channel());
		...
		if(discardLen>0)
		{
			...
			{
				logger.info("epChannelHandler,decode discard msg:{},channel:{}",WmIce104Util.ConvertHex(dicardBytes, 0),channelHandlerContext.channel());
			}
			else
			{
				logger.debug("epChannelHandler,decode discard msg:{},channel:{}",WmIce104Util.ConvertHex(dicardBytes, 0),channelHandlerContext.channel());
			}
		...
		if(readableBytes<7)
		{
			logger.debug("epChannelHandler,decode 1 readableBytes:{},channel:{}",readableBytes,channelHandlerContext.channel());
			return;
		}
		...
		if(remain_len<msg_len )
		{
			...
			logger.debug("epChannelHandler,decode ep remain_len<msg_len,remain_len:{},channel:{}",remain_len,channelHandlerContext.channel());
