...
public class UserService {
	...
	public static ChargeCardCache convertCardToCache(TblChargeCard tabCardInfo)
	{
		...
		if( cardType== EpConstant.CARD_THIRD_NORMAL ||
			cardType == EpConstant.CARD_THIRD_PRIVATE||
			cardType == EpConstant.CARD_CO_CREDIT)
		{
	...
	public static CardAuthResp checkUserCard(String epCode,ChargeCardCache cardCache,String userPasswordMd5)
	{
		...
		if( cardType!= EpConstant.CARD_NORMAL && 
			cardType!=EpConstant.CARD_NORMAL_PRIVATE &&
			cardType!=EpConstant.CARD_CO_CREDIT &&
			cardType!=EpConstant.CARD_THIRD_NORMAL&&
			cardType!=EpConstant.CARD_THIRD_PRIVATE)
		{
			ret.setErrorCode(EpConstant.E_CARD_INVALID);
			return ret;
		...
		if(cardType==EpConstant.CARD_NORMAL && epCache.getCompany_number()>0)
		{
			//普通卡不能刷带公司标识的桩
			ret.setErrorCode(EpConstant.E_CARD_NO_RIGHT);
			return ret;
		...
		//范围卡
		if( cardType==EpConstant.CARD_NORMAL_PRIVATE||
			cardType==EpConstant.CARD_THIRD_PRIVATE||
			cardType==EpConstant.CARD_CO_CREDIT)
		{
		...
		if(cardType== EpConstant.CARD_CO_CREDIT )
		{
	...
	public static void handleSelfCard(EpCommClient epCommClient,String epCode,int epGunNo,String inCardNo,String userPasswordMd5,byte []cmdTimes)
	{
		...
		byte[] data = EpEncodeProtocol.do_card_auth_resq(epCode,epGunNo,
