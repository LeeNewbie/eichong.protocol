...
public class EpBespokeService {
	...
	public static int apiStopBespoke(long pkBespNo, String bespNo,
		...
		if(besp==null)//没有预约
		{
			return EpConstantErrorCode.CAN_NOT_STOP_BESPOKE;// 
		}
	...
	public static void endBespoke(String epCode,BigDecimal realBespAmt,BespCache bespCache,Date et)
	{
		...
		if(bespStatus == EpConstant.BESPOKE_STATUS_END || bespStatus == EpConstant.BESPOKE_STATUS_END)
		{
		...
		{
			bespCache.setStatus(EpConstant.BESPOKE_STATUS_END);
			...
			UserService.addAmt(accountId,addingAmt);
		}
		else
		{
			bespCache.setStatus(EpConstant.BESPOKE_STATUS_FIN_UNSTAT);
		}
		...
		if(payMode == EpConstant.P_M_FIRST)
		{
			RateService.addPurchaseHistoryToDB(realBespAmt,2,accountId,bespCache.getUserOrigin().getOrgNo(),"预约消费",epCode,"",bespCache.getBespNo());
		}
	}
	...
	public static void handleEpBespRet(EpCommClient epCommClient, EpBespResp bespResp,byte []time)
	{
		...
		logger.info("handleEpBespRet nResult:{},epCode:{},epGunNo:{},redo:{},bespokeNo:{},SuccessFlag:{}",new Object[]{
