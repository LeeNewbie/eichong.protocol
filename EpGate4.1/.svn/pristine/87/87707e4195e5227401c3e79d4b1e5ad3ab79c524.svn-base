...
public class EpConcentratorService {
	...
	public static EpConcentratorCache InitStation(EpCommClient commClient,int stationId,int commVersion,int bootStatus)
	{
		...
		if(epList==null || epList.size()<=0)
		{
			logger.error("initConnect,stationId:{},commVersion:{},bootStatus:{},fail, have not TblElectricPile",
	...
	public static boolean initStationConnect(int commVersion,short nStationId,EpCommClient epCommClient,int boot)
	{
		//
		if(epCommClient ==null )
		{
			logger.error("initConnect,stationId:{} fail commClient ==null");
			return false;
		}
		...
		{
			EpCommClientService.handleOldClient(epCommClient, strStationId);
			
		}
		else
		{
			epStation = InitStation(epCommClient,nStationId,commVersion,boot);
			
		}
		if(epStation==null)
		{
			logger.error("initConnect,stationId:{} fail commVersion:{},channel:{},boot:{}",
					new Object[]{nStationId,commVersion,epCommClient.getChannel(),boot});
			return false;
		}
		
		int gateId = CoreConfig.gameConfig.getId();
		
		epCommClient.initNetSuccess(strStationId, commVersion, EpConstant.COMM_MODE_CENTRATOR);
	
		epStation.setEpNetObject(epCommClient);
		epStation.updateNetObject(epCommClient);
		...
		logger.info("initConnect success,stationId:{},getId:{},commVersion:{},boot:{},epNum:{},channel:{}",
  				new Object[]{nStationId,gateId,commVersion,boot,epStation.getEpCount(),epCommClient.getChannel()});
	...
	public static void handleCommClientTimeOut(String clientIdentity,int gateId)
	{
		...
		{
			logger.info("epChannelHandler,handleCommClientTimeOut fail, clientIdentity is empty");
			return;
		}
		...
		logger.info("epChannelHandler,handleCommClientTimeOut  modify CommStatus=0 to db,clientIdentity:{}",clientIdentity);
	}
