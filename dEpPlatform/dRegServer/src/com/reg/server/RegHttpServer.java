 ...
 */
public class RegHttpServer  extends AbstractHttpServer {
	
	private static final Logger logger = LoggerFactory.getLogger(GateManageServer.class);
	
	public RegHttpServer(ServerConfig serverConfig) {
		super(serverConfig);
	}
	...
	{
		try {
			return RegHttpServerHandler.handleGetMessage( path, params);
		} catch (IOException e) {
	...
	{
		try {
			return RegHttpServerHandler.handlePostMessage( postMethod, params);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return "";
