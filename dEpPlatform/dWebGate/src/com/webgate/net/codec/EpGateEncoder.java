...
import com.webgate.protocol.PhoneConstant;
...
public class EpGateEncoder extends MessageToByteEncoder{
	...
	private static final Logger logger = LoggerFactory.getLogger(EpGateEncoder.class + BaseConstant.SPLIT + GameConfig.serverName);
	...
	public static byte[] charge(byte[] hmsTime,String epCode,int epGunNo,long usrId,int frozenAmt,int changeMode,int showPrice)
	{
		DynamicByteBuffer byteBuffer = DynamicByteBuffer.allocate();
	...
	public static byte[] stopCharge(byte[] hmsTime,String epCode,int epGunNo,long usrId)
	{
		DynamicByteBuffer byteBuffer = DynamicByteBuffer.allocate();
