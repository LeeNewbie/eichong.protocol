...
import com.epcentre.server.EpMessageHandler;
...
public class ServerStop {
	...
	public static void shutDown(int shutdownPoint){
		...
		try {
			...
			logger.error("shutDown1 exception,e.getMessage:"+e.getMessage());
		}
		
		try {
			Thread.sleep(3000);
		}catch (InterruptedException e) {
			logger.error("shutDown exception,e.getMessage:"+e.getMessage());
		}
	}
	...
	public static int getShutDownPort(){
		...
		try {
           ...
           // System.out.println("关闭端口："+shutDownPort);
			...
			logger.error("getShutDownPort exception,e.getMessage:"+e.getMessage());
		}
	...
	public static Document getW3CDocument(String fileName){
		...
		try{
        	...
        	logger.error("getW3CDocument exception,e.getMessage:"+e.getMessage());
        }
