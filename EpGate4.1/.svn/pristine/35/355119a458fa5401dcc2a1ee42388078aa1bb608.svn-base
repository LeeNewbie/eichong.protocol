...
public class EqVersionService {
	...
	public static  void AddBomList(Integer key,Map<String,BomListInfo> bomList)
	{
		if(key !=null && bomList!=null)
		{
			...
			logger.debug("[handleVersion],updateHexFileMapsize:{},bomListsize:{}",updateHexFileMap.size(),bomList.size());
		}
		else
		{
			logger.error("[handleVersion],AddBomList fail,because of,key:{},bom:{}",key,bomList);
		}
	}
	public static  Map<String,BomListInfo>  getBomList(Integer key)
	{
		...
		{
			logger.error("[handleVersion],getBomList fail,because of,key:null");
			return null;
		}
	}
	...
	public static void getBomListByTypeSpanId(int typeSpanId)
	{
	    ...
	    {
			logger.info("[handleVersion], getTypeSpanAndBomFromDB TypeSpanId:{} had not TblBomList",typeSpanId);
	    ...
	    for (int j = 0; j < tblBomList.size(); j++) 
	    {
			...
			{
				logger.error("[handleVersion], getBomListByTypeSpanId bom error,hardwareVersion:{},softVersion:{}",
	...
	public static void getTypeSpanAndBomFromDB()
	{
		...
		{
			logger.error("[handleVersion], getTypeSpanAndBomFromDB had not TblTypeSpan");
			return ;
		}
		int size = tblTypeSpanList.size();
		logger.info("[handleVersion], getTypeSpanAndBomFromDB had {} TblTypeSpan",size);
	...
	{
		if(epCache ==null){
			logger.error("[handleVersion], getEpVersionFromDB epCache==null");
			return;
		}
		
		if(type!=1)
		{
			logger.error("[handleVersion], getEpVersionFromDB epCode:{},invalid type:{}",epCache.getCode(),type);
			return;
		}
	   ...
	   {
		   logger.info("[handleVersion], getEpVersionFromDB did not TblEquipmentVersion,epCode:{},pkEpId:{}",epCache.getCode(),epCache.getPkEpId());
		   return ;
	   }
	   logger.info("[handleVersion], getEpVersionFromDB epCode:{},pkEpId:{},had {} TblEquipmentVersion",
	...
	public static void getConcentratorVersionFromDB(EpConcentratorCache stationCache,int type)
	{
		...
		{
			logger.error("[handleVersion], getConcentratorVersionFromDB stationCache==null");
		...
		if(type!=2)
		{
			logger.error("[handleVersion], getConcentratorVersionFromDB stationAddr:{},invalid type:{}",stationAddr,type);
			return;
		}
		TblEquipmentVersion equitment = new TblEquipmentVersion();
		...
		if(equitMentVerList == null || equitMentVerList.size()<1)
		{
			logger.info("[handleVersion], getConcentratorVersionFromDB did not TblEquipmentVersion,stationAddr:{}",stationAddr);
			return ;
		}
		logger.info("[handleVersion], getConcentratorVersionFromDB stationAddr:{},had {} TblEquipmentVersion",stationAddr,equitMentVerList.size());
	 ...
	 public  static void saveEqVersiontoDB(EqVersionInfo verinfo,int pkId,int type,EqVersionCache eqVerCache)
	 {
		 ...
		 if(ver !=null )
		 {
					...
					ver.getSoftNumber().compareTo(verinfo.getSoftNumber())!=0)
			 {
				 ...
				 logger.info("[handleVersion], updateEqVersionToDB pkId:{},type:{}",pkId,type);
			 }
		 ...
		 else if(ver == null)
		 {
			 ...
			 logger.info("[handleVersion], insertEqVersionToDB pkId:{},type:{}",pkId,type);
	...
	public static void handleVersionAck(EpCommClient epCommClient, String epCode,int stationAddr,Vector<EqVersionInfo> verInfos) 
	{
		...
		logger.debug("[handleVersion]response of query ep version enter,epCode:{},stationAddr:{}",epCode,stationAddr);
		if(verInfos == null)
	    {
		    logger.error("[handleVersion]response of query ep version fail verInfos==null, epCode:{},stationAddr:{}",epCode,stationAddr);
		    return;
	    }
		...
		if(stationAddr>0) //处理站
		{
			...
			if(stationCache ==null)
			{
				 logger.error("[handleVersion]response of query ep version fail stationCache ==null, stationAddr:{}",stationAddr);
				return;
			}
		...
		else
		{
		    ...
		    if(epCache ==null)
		    {
			    logger.error("[handleVersion]response of query ep version fail epCache ==null, epCode:{}",epCode);
			    return;
		    }
		...
		forceUpdateHexFile(epCommClient,epCode,stationAddr, eqVerCache);
	}	
    ...
    public static void handleUpdateAck(int stationAddr,String epCode,EqVersionInfo info) 
    {
    	if(info ==null)
		{
    		logger.error("[handleVersion], response of update Over  fail, info ==null epCode:{},stationAddr:{}",epCode,stationAddr);
           return;
		}
		...
		if(stationAddr>0)//站
    	{
    		...
    		if(stationCache ==null)
    		{
    			logger.error("[handleVersion],response of update Over  fail, epCode:{},stationAddr:{}stationCache ==null",epCode,stationAddr);
    			return;
    		}
    		pkId = stationCache.getPkId();
    		eqVerCache=stationCache.getVersionCache();
		...
		else//电桩
    	{
    		...
    		if(epCache ==null)
    		{
    			logger.error("[handleVersion],response of update Over  fail epCache ==null epCode:{},stationAddr:{}",epCode,stationAddr);
    			return;
    		}
    		pkId = epCache.getPkEpId();
    		eqVerCache=epCache.getVersionCache();
    ...
    public static int sendVersion(EpCommClient epCommClient,String epCode, int stationId)
    {
	    ...
	    {
		   logger.error("[handleVersion], send getVersion to ep fail,epCommClient is not comm,epCode:{},stationAddr:{}",epCode,stationId);
		   return  1;
		
	    }
	    
	    int errorCode= queryVersion((EpCommClient)epCommClient,epCode,stationId); 
	    
	    logger.error("[handleVersion], send getVersion to ep success,epCode:{},stationAddr:{},errorCode:{}",
    ...
    {
    	logger.info("[handleVersion], queryEpVersion,typeSpanId:{},epCode:{}",typeSpanId,epClient.getCode());
    ...
    {
    	logger.info("[handleVersion], queryConcentratorVersion,typeSpanId:{},stationAddr:{}",typeSpanId,concentratorClient.getPkId());
    ...
    public  static void queryAllEpByTypeSpanID(int typeSpanId,Map<String,BomListInfo> bomMap)
    {
    	if(bomMap==null)
    	{
    		logger.error("[handleVersion],queryAllEpByTypeSpanID fail,bomMap is null typeSpanId:{}",typeSpanId);
	   		 return ;
    	}
   	 	List<TblElectricPile> epPileList=DB.epClientDao.findResultObjectBySpanId(typeSpanId);
	   	if(epPileList ==null ||epPileList.size()<1){
	   		 logger.error("[handleVersion],queryAllEpByTypeSpanID fail,not find ep from DB typeSpanId:{}",typeSpanId);
	   		 return ;
	   	}
	   	for(int i=0;i<epPileList.size();i++)
	   	{
	   		...
	   		if(epClient == null ){
	   			logger.error("[handleVersion],queryAllEpByTypeSpanID fail,epClient = null,epCode:{}",ep.getEpCode());
			   continue;
	   		}
	   		//清除电桩内存中的bom
     ...
     public synchronized static int queryAllEpVerion(int typeSpanId){
 		...
 		while (iter.hasNext()) {
	    	...
	    	if(epClient == null )
		    {
	    		logger.error("[handleVersion],queryAllEpVerion fail not find epClient typeSpanId:{}",typeSpanId);
			   continue;
		    }
	    	
	    	if(typeSpanId!=epClient.getTypeSpanId())
	    	{
	    		logger.error("[handleVersion],queryAllEpVerion fail typeSpanId:{} != epClient.getTypeSpanId():{},epCode:{}",
   ...
   //查询标志，queryFlag=1查询所有集中器，queryFlag=0查询指定产品类型集中器
     public synchronized static int queryAllStationVerion(int typeSpanId){
 		...
 		while (iter.hasNext()) {
		    ...
		    {
	    		logger.error("[handleVersion],queryAllStationVerion fail,stationClient= null,typeSpanId:{}",typeSpanId);
			   continue;
		    }
	    	
	    	if(typeSpanId !=stationClient.getTypeSpanId())
	    	{
	    		logger.error("[handleVersion],queryAllStationVerion fail,stationAddr:{},typeSpanId:{}",stationClient.getPkId(),typeSpanId);
	    		continue;
	    	}
    ...
    public static void forceUpdateHexFile(EpCommClient epCommClient,String epCode,int stationAddr,EqVersionCache eqVerCache)
 	{
    	...
    	{
    		 logger.info("[handleVersion],forceUpdateHexFile fail,epCode:{},stationAddr:{}",epCode,stationAddr);
    		 return;
    	}
    	if(eqVerCache==null || eqVerCache.getMapEpVersion()==null)
    	{
    		logger.info("[handleVersion],forceUpdateHexFile fail,not find Version, epCode:{},stationAddr:{}",epCode,stationAddr);
    		return;
    	}
        ...
        //该电桩下所有的升级版本和电桩现有版本对照判断
 		...
 		while (iter.hasNext()) {
 			...
 			if(verinfo==null)
 			{
 				logger.error("[handleVersion],forceUpdateHexFile fail,not find hardware in ep,epCode:{},stationAddr:{},key:{}",
    		...
    		if(bom==null)
    		{//电桩上无该硬件
    			logger.error("[handleVersion],forceUpdateHexFile fail,not find hardware from bom," +
    		...
    		{
    			logger.error("[handleVersion],forceUpdateHexFile fail,softversion is same," +
    		...
    		if(bom.getForceUpdate()==0)//不强制，判断版本高低，升级版本高就升
    		{
                ...
                {
                	logger.error("[handleVersion],forceUpdateHexFile fail,bomversion is Low,bom hardwareNumber:{}," +
                    			"hardwareVersion:{},epCode:{}," +
                    			"stationAddr:{},bom softVersion:{},ep softVersion:{}",
    ...
    public static int forceUpdateHexFile(EpCommClient epCommClient,String epCode, int stationAddr,String hardwareNumber,int hardwareM,int hardwareA,String bomSoft)
	{
		if(epCommClient==null || !epCommClient.isComm())
		{
			 logger.debug("[handleVersion],forceUpdateHexFile fail,commClient close,epCode:{},stationAddr:{}",epCode,stationAddr);
			 return 1;
		}
		try 
		{
		     ...
		     if(msg ==null)
		     {
			      logger.error("[handleVersion],forceUpdateEpHex fail,msg ==null,epCode:{},stationAddr:{}",epCode,stationAddr);
			      return 1;
		     }
	         byte[] cmdTimes = WmIce104Util.timeToByte();
		     EpMessageSender.sendMessage(epCommClient,0,0,Iec104Constant.C_FORCE_UPDATE_EP_HEX, msg,cmdTimes,epCommClient.getVersion());
		
		     logger.info("[handleVersion],forceUpdateHexFile success, send hardwareNumber:{},hardwareVersion:{},bomSoft:{},epCode:{},stationAddr:{}",
		...
		{ 
			logger.error("[handleVersion],forceUpdateEpHex exception,e.getMessage():{}",e.getMessage());
			return 1;
		}
			...
			String hardwareNumber,int hardwareM,int hardwareA,int len,byte []cmdTimes)
	{
	   ...
	   if(epCommClient==null || !epCommClient.isComm())
	   {
		    logger.info("[handleVersion],handleEpHexFileSumaryReq fail,commClient==null || commClient is close,epCode:{},stationAddr:{}",
					epCode,stationAddr);
			return;
	   }
		if (stationAddr ==0 && epCode.compareTo("0000000000000000")==0 )
    	{
			logger.info("[handleVersion],handleEpHexFileSumaryReq fail,did not find epCode:{},stationAddr:{}",
					epCode,stationAddr);
			 return ;
    	}
		...
		if(bom == null)
		{
		    ...
		    logger.info("[handleVersion],handleEpHexFileSumaryReq fail,not find bom,epCode:{},stationAddr:{},key:{}",
				new Object[]{epCode,stationAddr,key});
		    return;
		}
         ...
         if(file_len >0)
         {
			 ...
			 existFlag=1;
			 updateFlag=1;
			 logger.info("[handleVersion], handleEpHexFileSumaryReq success, filename:{},epCode:{},stationAddr:{}",
						new Object[]{filename,epCode,stationAddr});
		 }
         else 
		 {
        	 file_len=0;
			 logger.info("[handleVersion],handleEpHexFileSumaryReq fail,not find filename:{},epCode:{},stationAddr:{}",
						new Object[]{filename,epCode,stationAddr});
		 }
		 ...
		 if (msg == null) 
		 {
			 logger.info("[handleVersion],handleEpHexFileSumaryReq send fail,exception");
			 return;
		 }
	...
	public static BomListInfo getBomInfo(int stationAddr,String epCode,String hardwareNumber,int hardwareM,int hardwareA)
	{
		...
		if(stationAddr>0)
		{
			...
			if(stationCache==null)
			{
				logger.info("[handleVersion],getBomInfo fail,not find EpStationCache,stationAddr:{}",stationAddr);
				return null;
			}
		 ...
		 else
		 {
			...
			if(epCache == null)
			{
				logger.info("[handleVersion],getBomInfo fail,not find epCache,epCode:{}",epCode);
				return null;
			}
	...
	public static void handleEpHexFileDownReq(EpCommClient epCommClient, String epCode,short stationAddr,
		...
		if (stationAddr == 0 && epCode.compareTo("0000000000000000") == 0) {
			logger.error("[handleVersion],handleEpHexFileDownReq fail, did not find eq,epCode:{},stationAddr:{}",
					epCode,stationAddr);
			return;
		}
		...
		{
			logger.error("[handleVersion],handleEpHexFileDownReq fail,commClient is close,epCode:{},stationAddr:{}",
					epCode,stationAddr);
			return;
		}
		...
		{
			len=0;
			logger.error("[handleVersion],handleEpHexFileDownReq fail,len<=0,epCode:{},stationAddr:{},softNumber:{},softVersion:{}",
		...
		if(msg ==null)
		{
			logger.error("[handleVersion],handleEpHexFileDownReq fail,do_ep_hex_file_down exception");
			return ;
		}
	...
	public static int queryVersion(EpCommClient commClient,String epCode,int stationAddr)
	{
     ...
     { 
    	 if(bomMap==null)
     	 {
     		logger.error("[handleVersion],queryAllStaionByTypeSpanID fail,bomMap is null typeSpanId:{}",typeSpanId);
 	   		 return ;
     	 }
   	     List<TblConcentrator> centList=DB.concentratorDao.findResultObjectBySpanId(typeSpanId);
   	     if(centList ==null ||centList.size()<1)
    	 {
    		 logger.error("[handleVersion],queryAllStaionByTypeSpanID fail,not find concentrator from DB typeSpanId:{}",typeSpanId);
    	 }
    	 for(int i=0;i<centList.size();i++)
    	 {
    		 ...
    		 if(stationClient == null )
 		     {
 	    		logger.error("[handleVersion],queryAllStaionByTypeSpanID fail,stationClient = null stationAddr:{}",
