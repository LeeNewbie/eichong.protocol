...
public class EleasePush extends Push{
	...
	private static final Logger logger =  LoggerFactory.getLogger(EleasePush.class);
	...
	public int getIntValue(Map<String ,Object> realData,String key)
	{
	...
	public String getStringValue(Map<String ,Object> realData,int key)
	{
		Object o = realData.get(key);
		String value="";
		if(o!=null) value = (String)o;
		return value;
	}
	
	public BigDecimal intToBigDecimal3(int i)
	{
		BigDecimal value = new BigDecimal(i).multiply(new BigDecimal(0.001));
		value = value.setScale(3,BigDecimal.ROUND_HALF_UP);
		return value;
	}
	public BigDecimal intToBigDecimal2(int i)
	{
		BigDecimal value = new BigDecimal(i).multiply(new BigDecimal(0.01));
		value = value.setScale(2,BigDecimal.ROUND_HALF_UP);
		return value;
	}
	public BigDecimal intToBigDecimal1(int i)
	{
		BigDecimal value = new BigDecimal(i).multiply(new BigDecimal(0.1));
	...
	@Override
	public boolean init() {
		boolean b=ConfigManager.getMessageManager().init();
		
		this.setSendParams(Consts.orgNo,Consts.mode,Consts.period);
		return b;
	}
	...
	public void onChargeEpResp(String token,int orgNo,String userIdentity,String epCode,int epGunNo, String extra,int ret,int errorCode) {
		...
		ELeaseService.sendChargeResp(resMap);
	}
	...
	public void onStopChargeEpResp(String token,int orgNo,String userIdentity,String epCode,int epGunNo, String extra,int ret,int errorCode) {
		...
		ELeaseService.sendStopChargeResp(resMap);
	}
	
	@Override
	public void onRealData(String token,int orgNo,String userIdentity,String epCode,int epGunNo,int inter_type,Map<String ,Object> realData,String extra) {
		
		JSONObject jsonObject = JSONObject.fromObject(realData);
		...
		int inter_conn_state= getIntValue(realData,"1_1");
		resMap.put("inter_conn_state", convertGunStauts(inter_conn_state));
		
		//
		int inter_work_state= getIntValue(realData,"3_1");
		...
		int v= getIntValue(realData,"3_3");
		if(v>=0)
			resMap.put("voltage", intToBigDecimal1(v));
		
		v= getIntValue(realData,"3_4");
		if(v>=0)
		resMap.put("current", intToBigDecimal2(v));
		
		resMap.put("soc", getIntValue(realData,"3_5"));//soc 是 int 当前的 SOC（ 汽车电量的百分比）
		...
		resMap.put("time", getCurrentSeconds());//time 是 int 上报时间（ 秒格式 Unix 时间戳）
		
		ELeaseService.sendRealData(resMap);
	}
			...
			,int start_time,int end_time,int stop_model,int stop_reason,int soc,int time,String extra) 
	{
		...
		ELeaseService.sendOrderInfo(resMap);
