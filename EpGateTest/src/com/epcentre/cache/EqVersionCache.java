...
public class EqVersionCache {
	...
	//保存桩需要升级的硬件清单
	private  Map<String,BomListInfo> mapBomList ;
	...
	public  void setBomMap(Map<String,BomListInfo> bomMap)
	{
		mapBomList = bomMap;
	}
	...
	public   void AddBomList(String key,BomListInfo bomList)
	{
		...
		{  
			if(mapBomList == null)
			{
				mapBomList = new ConcurrentHashMap<String,BomListInfo>();
			}
	...
	public  void removeEpVersion(Vector<EqVersionInfo> verInfos)
	{
		...
		while (iter.hasNext()) {
			...
			if(result==0)//找不到相同的硬件，删除数据库中保存的记录
			{
				...
				EqVersionService.deleteEqVersionFromDB( ver.getPk_EquipmentVersion());
		...
		final StringBuilder sb = new StringBuilder();
    	 Iterator iter = mapBomList.entrySet().iterator();	
 		 while (iter.hasNext()) {
 	        ...
 	        sb.append("{");
 	        sb.append("硬件编号 = ").append(ver.getHardwareNumber());
 	        sb.append("硬件版本号 = ").append(ver.getHardwareVersion());
 	        sb.append("固件编号 = ").append(ver.getSoftNumber());
 	        sb.append("固件版本号 = ").append(ver.getSoftVersion());
