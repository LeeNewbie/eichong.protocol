 ...
 */
public class MonitorHttpServer extends AbstractHttpServer {

	private static final Logger logger = LoggerFactory.getLogger(MonitorHttpServer.class);
	
	public MonitorHttpServer(ServerConfig serverConfig) {
		super(serverConfig);
	}
	@Override
	...
	{
		try {
			return MonitorHttpServerHandler.handleGetMessage( path, params);
		} catch (Exception e) {
	...
	{
		try {
			return MonitorHttpServerHandler.handlePostMessage( postMethod, params);
		} catch (Exception e) {
			// TODO Auto-generated catch block
