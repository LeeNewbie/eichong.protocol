...
public class UsrGateService {
			...
			int bDispPrice,String carNo,String carVin)
	{
	    ...
	    if(orgNo !=UserConstants.ORG_I_CHARGE)
		{
	    	...
	    	usrId=UserService.getUserIdByOrgNo(orgNo);
			...
			int bDispPrice,String carNo,String carVin)
	{
		...
		else
		{
	        ...
	        if(errorCode>0)
	        {
	        	...
	        	successflag =  0;
	           	data = UsrGateEncoder.charge(h,m,s,epCode,epGunNo,OrgNo,usrLog,token,successflag, (short)errorCode);
	
	        }
	        else
	        {
	        	data = UsrGateEncoder.ack((short)U2ECmdConstants.EP_CHARGE,h,m,s,OrgNo,usrLog,token);
			...
			int epGunNo,int OrgNo,String usrLog,String token)
	{
		...
		else
		{
		    ...
		    { //失败
		        ...
		        String extraData="";
				data =  UsrGateEncoder.stopCharge(h, m, s, epCode, epGunNo,OrgNo,usrLog,
	  	        		 token, successflag, (short)errorCode);
		    } 
			else
			{
				data = UsrGateEncoder.ack((short)U2ECmdConstants.EP_STOP_CHARGE,h,m,s,OrgNo,usrLog,token);
			}
		}
		if(data!=null)
		{
