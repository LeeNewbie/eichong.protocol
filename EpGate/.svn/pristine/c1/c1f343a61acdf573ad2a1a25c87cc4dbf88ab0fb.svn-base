...
public class EpEncodeProtocol {
	...
	public static  byte[] PackageI(byte[] data,byte type,short cos,int sendINum,int recvINum,int common_address,byte record_type
			,byte []time,int commVersion) 
	{
		try
		{ 	
			ApciHeader apciHeader = new ApciHeader();
			apciHeader.setLength(ApciHeader.getHLen() + AsduHeader.H_LEN + 1 + data.length- Iec104Constant.INITIAL_BYTES_TO_STRIP);
			...
			if(commVersion>=EpProtocolConstant.PROTOCOL_VERSION_V4)
			{
				...
				short crc =record_type;
				crc += WmIce104Util.CRCSum(data);
				crc += WmIce104Util.CRCSum(time);
	...
	public static  byte[] PackageI(byte[] data,byte type,short cos,int sendINum,int recvINum,int common_address,byte []time,int commVersion) {
		try{
			ApciHeader apciHeader = new ApciHeader();
			...
			if(commVersion>=EpProtocolConstant.PROTOCOL_VERSION_V4)
			{
				...
				short crc = WmIce104Util.CRCSum(data);
				crc += WmIce104Util.CRCSum(time);
				bmsg.write(WmIce104Util.short2Bytes(crc));
			}
	...
	public static  byte[] PackageI(byte type,short cos,int sendINum,int recvINum,int common_address
			,byte []time,int commVersion) {
		try{
		ApciHeader apciHeader = new ApciHeader();
		apciHeader.setLength(ApciHeader.getHLen() + AsduHeader.H_LEN - Iec104Constant.INITIAL_BYTES_TO_STRIP);
		apciHeader.setINr(sendINum, recvINum);
		...
		if(commVersion>=EpProtocolConstant.PROTOCOL_VERSION_V4)
		{
			...
			short crc= WmIce104Util.CRCSum(time);
			bmsg.write(WmIce104Util.short2Bytes(crc));
		}
	...
	public static byte[] do_set_time(short cos,int sendINum,int recvINum,int common_address,
		...
		byte time[] = WmIce104Util.timeToByte();
		 return PackageI(data,Iec104Constant.C_CS_NA,cos,sendINum,recvINum,common_address,time,commVersinon);
	}
	...
	public static byte[] Package_all_call(short cos,int sendINum,int recvINum,int common_address,
		...
		data[0]=20;
		byte time[] = WmIce104Util.timeToByte();
		return PackageI(data,Iec104Constant.C_IC_NA,cos,sendINum,recvINum,common_address,time,commVersinon);
	}
	/***
	 * 
	 * @param cos
	...
	public static  byte[] do_nocard_auth_by_yzm(short cos,int sendINum,int recvINum,int common_address,String epCode,int nInterFace,byte bSucess,String strSucessDesc, 
		...
		try{	
			...
			return PackageI(bout.toByteArray(),Iec104Constant.C_SD_NA,cos,sendINum,recvINum,common_address,
					(byte)Iec104Constant.C_NOCARD_AUTH,time,commVersion);
		}
		catch (IOException e) 
		{
			e.printStackTrace();
