 ...
 * handler
 */
public class ChatServerChannelHandler extends ChannelHandlerAdapter {

    AgreementMsg msgAgree = new AgreementMsg(true);

    /**
     * 通道已注册
     * 用户允许从EventLoop中注销Channel暂停事件执行,然后再重新注册
     * 所以此状态变化可以出现多次
     * @param ctx
     * @throws Exception
     */
    ...
    public void channelRegistered(ChannelHandlerContext ctx) throws Exception {
        System.out.println(ctx.channel().getClass().getName() + " 通道已注册...");
    }

    /**
     * 通道已活动 此状态在channel生命周期内只有一个
     * @param ctx
     * @throws Exception
     */
    @Override
    ...
    public void channelUnregistered(ChannelHandlerContext ctx) throws Exception {
        System.out.println(ctx.channel().getClass().getName() + " 通道已注销...");
    }
