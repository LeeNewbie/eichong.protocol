...
public class EpGateDecoder extends ByteToMessageDecoder {
	...
	protected void decode(ChannelHandlerContext channelHandlerContext,
		...
		if(discardLen>0)
		{
			...
			{
				// 接收字节数累加
				CacheService.setEpFlowStat(channelHandlerContext.channel(), discardLen, false);
				//没有数据可对，还回
				return;
			}
		}
		...
		if(remain_len<msg_len )
		{
			...
			// 接收字节数累加
			CacheService.setEpFlowStat(channelHandlerContext.channel(), discardLen+4, false);
    	...
    	list.add(message);
		// 接收字节数累加
		CacheService.setEpFlowStat(channelHandlerContext.channel(), discardLen+msg_len+4, false);
	}
