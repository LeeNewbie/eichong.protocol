...
public class RequestUtil {
	...
	private static URI generateURL(String url, Map<String, Object> params) {
		...
		try {
			...
			uri = uriBuilder.build();
			 ...
			 logger.error("generateURL exception,e.getMessage():{}",e.getMessage());
		}
		return uri;
	}
	...
	private static String execute(HttpUriRequest request) {
		...
		try {
			...
			responseStr = EntityUtils.toString(response.getEntity(),
			 ...
			 logger.error("execute exception,e.getMessage():{}",e.getMessage());
		} finally {
	...
	public static HttpClient wrapClient(HttpClient base) {
		try {
			...
			logger.error("wrapClient exception,e.getMessage():{}",ex.getMessage());
			return null;
		}
	}
}
